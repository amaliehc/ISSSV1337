...
output: 
  flexdashboard::flex_dashboard:
    ...
    theme: lumen
    source_code: https://github.com/torbjornso/ISSSV1337group5
runtime: shiny
---
...
Please upload an html copy of annual accounts from [Proff.no](https://www.proff.no).

Then upload an Excel file in the [set format](https://github.com/torbjornso/ISSSV1337group5/blob/27d1d4c25882091d6aef51956000f7ef3fefff75/Dashboard/Data/NGO_template.xltx).
...
- **Fundraising ROI**:
  - A basic measure of the return on investment of fundraising expenses: 
...
html_test_create <- function(file) {
...
read_proff_one <- function(object) {
  
  html1 <- object |>
    # removing empty column, using all_of(argument)
    select(
      1:(length(names(object)) - 1)
    ) |>
